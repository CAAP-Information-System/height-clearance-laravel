...
class ATSController extends Controller
{
     ...
     */
    public function ATSChiefHome()
    {
        $queue = Application::all();
        return view('ats.chief.home', compact('queue'));
    }
    ...
    public function ATSEvalUpdate(Request $request, $id)
    {
        $user = Auth::user();
        ...
        $queue_status->ats_eval = 'Evaluated';
        $queue_status->ats_supervisor = 'For Review';
        $queue_status->save();

        return redirect()->route('ATStoSupervisor', ['id' => $user->id]);
    }

    public function proceedToSupervisor()
    {
        return view('ats.proceed_message.proceed_to_supervisor',);
    }

    public function ATSSupervisor(Request $request, $id)
    {
        $user = Auth::user();
        $applicationData = Application::find($id);
        ...
        if ($applicationData) {
            ...
            return view(
                'ats.supervisor.evaluation-form',
                compact('applicationData', 'user', 'userData', 'airports', 'files', 'receipt', 'aerodrome')
            )
                ->with('fileNameToStore_elevation_plan', $fileNameToStore_elevation_plan)
                ->with('fileNameToStore_geodetic_eng_cert', $fileNameToStore_geodetic_eng_cert)
    ...
    public function ATSSupervisorUpdate(Request $request, $id)
    {
        $user = Auth::user();
        ...
        // Updates the process status that the application is finished being evaluated.
        $queue_status->ats_supervisor = 'Checked';
        $queue_status->ats_chief = 'For Review';
        $queue_status->save();

        return redirect()->route('ATStoChief', ['id' => $user->id]);
    }
    public function proceedToChief()
    {
        return view('ats.proceed_message.proceed_to_chief',);
    }
    public function ATSChiefView(Request $request, $id)
    {
        $user = Auth::user();
    ...
    public function ATSChiefUpdate(Request $request, $id)
    {
        $user = Auth::user();
        // Check if the ApplicationQueue record exists
        $queue_status = ApplicationQueue::where('user_id', $id)->first();
